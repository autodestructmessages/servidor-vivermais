// VIVERMAIS V4.3.4 — SERVIDOR DE CHAT, PUSH CONFIÁVEL, RANKING E WEBRTC DIRECIONADO
...
const TEMPO_REENTRADA_SEM_PUSH_MS = 2 * 60 * 1000;
 ...
 */
/**
 * Mascara um token para logs sem expor o valor completo.
 */
function descreverTokenPush(token) {
  if (!token || typeof token !== 'string') return 'ausente';
  const limpo = token.trim();
  if (limpo.length <= 16) return 'presente';
  return `${limpo.slice(0, 12)}...${limpo.slice(-5)}`;
}

/**
 * Valida tokens gerados pelo Expo Push Service.
 * Mantém compatibilidade com os prefixos ExpoPushToken e ExponentPushToken.
 */
...
/**
 * Remove um token inválido de todas as salas em memória.
 * Isso evita novas tentativas para instalações que foram removidas do aparelho.
 */
function removerTokenDasSalas(tokenInvalido) {
  if (!tokenInvalido) return;

  Object.values(salasAtivas).forEach(sala => {
    if (!Array.isArray(sala?.tokens)) return;
    sala.tokens = sala.tokens.filter(token => token !== tokenInvalido);
  });
}

/**
 * Envia uma notificação mascarada pelo Expo Push Service.
 * Não usa channelId fixo: assim o Android pode usar o canal padrão mesmo se o
 * canal personalizado tiver sido apagado ou ainda não tiver sido criado.
 * Retorna os tickets para diagnóstico no Render sem expor tokens completos.
 */
async function enviarNotificacao(
  ...
  if (validTokens.length === 0) {
    console.log(
      `[PUSH] Nenhum Expo Push Token válido entre ${tokensDestino.length} destino(s).`
    );
    return {
      enviados: 0,
      ignorados: tokensDestino.length
    };
  }
  ...
  try {
    console.log(
      `[PUSH] Enviando ${validTokens.length} notificação(ões) mascarada(s) ao Expo.`
    );
    ...
    if (!resposta.ok) {
      console.log(
        `[PUSH] Expo respondeu HTTP ${resposta.status}: ${corpoResposta.slice(0, 800)}`
      );
      return {
        enviados: 0,
        ignorados: tokensDestino.length - validTokens.length,
        erroHttp: resposta.status
      };
    }
    ...
    try {
      respostaJson = JSON.parse(corpoResposta);
    } catch (erroJson) {
      console.log(
        '[PUSH] Resposta recebida, mas não foi possível interpretar os tickets:',
        erroJson?.message || erroJson
      );
    }

    const tickets = Array.isArray(respostaJson?.data)
      ? respostaJson.data
      : respostaJson?.data
        ? [respostaJson.data]
        : [];
    ...
    tickets.forEach((ticket, indice) => {
      const token = validTokens[indice];

      if (ticket?.status === 'ok') {
        aceitos += 1;
        console.log(
          `[PUSH] Ticket aceito para ${descreverTokenPush(token)}; id=${ticket.id || 'sem-id'}.`
        );
        return;
      }

      erros += 1;
      const codigoErro = ticket?.details?.error || 'erro_desconhecido';

      console.log(
        `[PUSH] Ticket recusado para ${descreverTokenPush(token)}; erro=${codigoErro}; mensagem=${ticket?.message || 'sem mensagem'}.`
      );

      if (codigoErro === 'DeviceNotRegistered') {
        removerTokenDasSalas(token);
        console.log('[PUSH] Token DeviceNotRegistered removido das salas em memória.');
      }
    });

    // Alguns retornos HTTP 200 podem vir sem lista de tickets. Ainda registramos
    // a resposta integral limitada para facilitar o diagnóstico no Render.
    if (tickets.length === 0) {
      console.log(
        `[PUSH] Resposta HTTP 200 sem tickets interpretáveis: ${corpoResposta.slice(0, 800)}`
      );
    } else {
      console.log(
        `[PUSH] Resultado do lote: ${aceitos} aceito(s), ${erros} erro(s).`
      );
    }

    return {
      enviados: aceitos || validTokens.length,
      erros,
      ignorados: tokensDestino.length - validTokens.length,
      tickets
    };
    ...
    console.log(
      '[PUSH] Falha de rede ao chamar Expo Push Service:',
      error?.message || error
    );
...
/**
 * Cria uma identidade estável e não legível para controlar reentradas.
 * Preferimos deviceId; o token é usado como fallback e o socket só em último caso.
 */
...
/**
 * Registra uma entrada e informa se já passaram dois minutos desde a última
 * entrada do MESMO dispositivo na MESMA sala.
 *
 * Importante: o timestamp é atualizado em toda entrada. Portanto várias
 * reentradas acidentais consecutivas continuam silenciosas até o usuário
 * permanecer pelo menos dois minutos sem entrar novamente.
 */
function registrarEntradaEVerificarPush(
  ...
  ultimaEntradaPorSalaDispositivo.set(chave, agora);

  // Limpeza defensiva para evitar crescimento indefinido.
  if (ultimaEntradaPorSalaDispositivo.size > 1000) {
    ...
    for (
      ...
      if (timestamp < limite) {
        ultimaEntradaPorSalaDispositivo.delete(chaveAntiga);
      }
    }
  }

  if (anterior && agora - anterior < intervaloMs) {
    console.log(
      `[PUSH] Reentrada em ${codigoSala} dentro de 2 min; alerta de entrada suprimido.`
    );
    return false;
  }
...
/**
 * Apenas marca a entrada atual sem disparar notificação.
 * Usado pelo criador da sala para que uma saída e reentrada imediatas também
 * respeitem a janela de dois minutos.
 */
...
/**
 * Retorna os tokens conhecidos da sala, removendo o token do próprio remetente.
 */
function obterTokensDaSalaExceto(
  codigoSala,
  tokenExcluido
) {
  const sala = obterSala(codigoSala);

  if (!sala || !Array.isArray(sala.tokens)) {
    return [];
  }

  return Array.from(
    new Set(
      sala.tokens.filter(token =>
        tokenExpoValido(token) &&
        token !== tokenExcluido
      )
    )
  );
...
/**
 * Retorna apenas os tokens que NÃO estão atualmente conectados à sala.
 * Assim uma mensagem nova gera push para quem saiu/bloqueou o app, mas não
 * duplica a experiência de quem já está lendo a conversa ao vivo.
 */
async function obterTokensOfflineDaSala(
  codigoSala,
  tokenRemetente,
  socketRemetenteId
) {
  const candidatos = obterTokensDaSalaExceto(
    codigoSala,
    tokenRemetente
  );
  ...
  try {
    ...
    const tokensOnline = new Set();

    socketsNaSala.forEach(item => {
      if (item.id === socketRemetenteId) return;

      const token = item.data?.tokenPush;
    ...
    return candidatos.filter(
      token => !tokensOnline.has(token)
    );
  } catch (erro) {
    console.log(
      `[PUSH] Falha ao identificar tokens online em ${codigoSala}; usando lista conhecida:`,
      erro?.message || erro
    );
...
/**
 * Dispara a notificação mascarada de nova mensagem somente para dispositivos
 * que não estão com a sala aberta naquele momento.
 */
async function notificarNovaMensagem(
  ...
  const tokensDestino =
    await obterTokensOfflineDaSala(
      codigoSala,
      tokenRemetente,
      socketRemetenteId
    );
  ...
  if (tokensDestino.length === 0) {
    console.log(
      `[PUSH] Mensagem em ${codigoSala}: nenhum destinatário offline para notificar.`
    );
    return;
  }
  ...
  await enviarNotificacao(
    tokensDestino,
    '🏆 Ranking atualizado!',
    'Há uma nova atualização disponível no ViverMais.',
    'atualizacao_ranking'
  );
}
...
io.on('connection', socket => {
  ...
  socket.on(
    ...
    async (
      ...
      // de segurança e voltar em menos de dois minutos, não geramos alerta duplicado.
  ...
  socket.on(
    ...
    async (
      ...
      // se já passaram dois minutos desde a última entrada desse mesmo aparelho.
      ...
      // A entrada só avisa os outros aparelhos quando:
      // 1) não foi mera troca de rede; e
      // 2) o mesmo dispositivo não entrou nessa sala nos últimos dois minutos.
      if (
        !reconexaoMesmoDispositivo &&
        foraDaJanelaDeReentrada
      ) {
        const tokensParaAvisar =
          obterTokensDaSalaExceto(
            codigoNormalizado,
            tokenPush
          );
        ...
        if (tokensParaAvisar.length > 0) {
          ...
          console.log(
            `[PUSH] Sala privada ${codigoNormalizado} ainda não possui outro token para avisar.`
          );
        }
      }
